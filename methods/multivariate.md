...
$$
D_B(z; \\{x_i\\}) = {\rm Avg} {\rm Avg}_k {\cal I}[{\rm min}(x_{j_1}(k), x_{j_2}(k), x_{j_3}(k)) <
    z(k) < {\rm max}(x_{j_1}(k), x_{j_2}(k), x_{j_3}(k))].
$$
...
95\% of the probability.  The volume of this ellipsoid is $t^p\cdot|\Sigma|^{1/2}$
