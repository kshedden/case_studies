...
Notably, this model can be fit using ordinary least squares, giving what is essentially a _linear probability model_.  For each time $t$ that is an observed event time ($t in {t_1, t_2, ..., t_m}$), let $X^t$ denote the design matrix based on the covariates at time $t$, including only subjects who are at-risk at time $t$.  Similarly, let $y^t$ denote a vector indicating all subjects who have the event at time $t$, also restricted to subjects at risk.  Let $hat(alpha)_t$ denote the ordinary least squares slopes for the linear regression of $y^t$ on $X^t$.  Then define $hat(beta)_t = sum_(s <= t) hat(alpha)_s$.  These $hat(beta)_t$ are estimates of the cumulative coefficient functions $beta_j (t)$.

The values of $frac(hat(alpha)_(t_j), (t_(j+1) - t_j), style: "horizontal")$ estimate the coefficient functions $alpha_j (t)$, but there is very little information available for this estimation -- if there are no ties, then only one failure occurs at each time $t_j$.  Therefore, the $frac(hat(alpha)_j (t_j), (t_(j+1) - t_j), style: "horizontal")$ do not concentrate well around the $alpha_j (t)$.  However the cumulative coefficients behave much better -- the $hat(beta)_j (t)$ do concentrate around the $beta_j (t)$.  The $hat(beta)_j$ are asymptotically normal and confidence bands can be calculated for them, although the procedure for doing so is somewhat complex.

A semiparametric #link("https://www.jstor.org/stable/2337122")[extension] of the Aalen additive hazards model allows some of the coefficient functions to be constrained to be constant.  Further, it is possible to gain a small amount of additional power by using weighted least squares (WLS) instead of ordinary least squares (OLS).  This takes account of the heteroscedasticity inherent in the fact that each $y^t$ has binary elements.
