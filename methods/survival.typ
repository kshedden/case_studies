...
The values of $hat(alpha)_(t_j) / (t_(j+1) - t_j)$ estimate the coefficient functions $alpha_j(t)$, but there is very little information available for this estimation -- if there are no ties, then only one failure occurs at each time $t_j$.  Therefore, the $hat(alpha)_j(t_j) / (t_(j+1) - t_j)$ do not concentrate well around the $alpha_j (t)$.  However the cumulative coefficients behave much better -- the $hat(beta)_j (t)$ do concentrate around the $beta_j (t)$.  The $hat(beta)_j$ are asymptotically normal and confidence bands can be calculated for them, although the procedure for doing so is somewhat complex.
