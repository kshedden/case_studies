...
```{r}
f4 = as.formula("BPXSY1 ~ te(RIDAGEYR, BMXBMI, k=c(3, 3)) + s(RIDAGEYR, by=RIAGENDR) + s(BMXBMI, by=RIAGENDR)")
m4 = gam(f4, data=dx)
plot_all(m4, 4)
```
...
```{r}
f5 = as.formula("BPXSY1 ~ te(RIDAGEYR, BMXBMI, by=RIAGENDR, k=c(3, 3)) + s(RIDAGEYR, by=RIAGENDR) + s(BMXBMI, by=RIAGENDR)")
m5 = gam(f5, data=dx)
plot_all(m5, 5)
```
