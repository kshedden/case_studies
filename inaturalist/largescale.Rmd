...
```{r}
dn = da %>% group_by(scientificName) %>% summarize(n=n())
...
```{r}
da = da %>% group_by(scientificName) %>% transform(lonrad_sin_cen=mean(lonrad_sin), lonrad_cos_cen=mean(lonrad_cos))
...
```{r}
f = function(x, y) {
  scientificName = as.character(y)
  if (nrow(x) < 10) {
    return(list(scientificName=scientificName, n=nrow(x)))
  }
  ...
  return(list(scientificName=scientificName, n=nrow(x), lrt_z=lrt_z, lrt_zwh=lrt_zwh,
              day_slope=day_slope, day_slope_se=day_slope_se,
              fake_slope=fake_slope, fake_slope_se=fake_slope_se))
}

rr = da %>% group_by(scientificName) %>% group_map(f) %>% bind_rows()
```
...
```{r}
rr = left_join(rr, dm, by="scientificName")
```
