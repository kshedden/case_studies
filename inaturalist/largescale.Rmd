...
```
Calculate the mean latitude for each species:
...
head(da)
```
Below we calculate the intraclass correlation coefficient (ICC), which describes the extent to which different species cluster by latitude.  An ICC of 1 corresponds to complete clustering and an ICC of 0 corresponds to no clustering.

```{r}
var(da$species_latitude) / var(da$decimalLatitude)
```

Analyze longitude as a circular variable.
...
f = function(x, y) {
  ...
  dof = lrt$Df[[2]]
  if (dof == 3) {
    # The Wilson and Hilferty transformation from chi-square to Gaussian.
