...
```{r}
da = da %>% group_by(species) %>% mutate(n=n()) %>% ungroup()
da = da %>% filter(n>=1000)
```
...
da = da %>% group_by(species) 
        %>% mutate(lonrad_cos_mean=mean(lonrad_cos),
                   lonrad_sin_mean=mean(lonrad_sin)) 
        %>% ungroup()
```
...
```{r}
da1 = da %>% group_by(species) %>% slice_head(n=1)
...
dd = da %>% group_by(species) %>% group_modify(~ f(.x)) %>% ungroup()
```
...
```{r}
xq = dd %>% select(species, q50)
xq = arrange(xq, q50)

for (i in 1:3) {
  p = map_species(xq$species[i])
  print(p)
}
