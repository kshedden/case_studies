...
Estimate and plot the baseline cumulative hazard function.  Cumulative hazard functions are easy to estimate but somewhat difficult to interpret.
...
Plot the baseline hazard function for each sex.  Note that the basehaz function in R returns the cumulative hazard but the column is labeled 'hazard'.

```{r}
bh = basehaz(m7)
print(bh)
plt = ggplot(bh, aes(x=time, y=log(hazard), color=strata)) + geom_line() + xlim(0, 100) + xlab("Age") + ylab("Log cumulative hazard")
print(plt)
```
